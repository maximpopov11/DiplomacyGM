...
def parse(
    ...
    for order in orders:
        try:
            if is_moves_phase(adjudicator.phase):
                ...
                valid.add(
                    _parse_order_adjustments(order, player_restriction, provinces)
                )
    ...
    if invalid:
        response = (
            "Some orders validated successfully. The following orders were invalid:"
        )
...
def _parse_order_moves(
    order: str, player_restriction: Player, provinces: Mapping[str, Province]
) -> Order:
    order = order.lower()
...
def _parse_convoy_move(
    ...
    if player_restriction is not None and position.unit.player != player_restriction:
        raise PermissionError(
            f"You, {player_restriction.name}, do not have permissions to order the unit in "
...
def _parse_build(
    ...
    if player_restriction is not None and province.unit.player != player_restriction:
        raise PermissionError(
            f"You, {player_restriction.name}, do not have permissions to build in "
...
def _parse_provinces(
    order: str, all_provinces: Mapping[str, Province], count: int
) -> List[Province]:
    provinces = []
    for word in order:
